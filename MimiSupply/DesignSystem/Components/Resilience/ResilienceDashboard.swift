...
struct ResilienceDashboard: View {
    ...
    private var keyMetricsGrid: some View {
        LazyVGrid(columns: [
            ...
            MetricCard(
                title: "Queue Size",
                value: "\(requestQueue.queuedRequests)",
    ...
    private var requestQueueOverview: some View {
        VStack(alignment: .leading, spacing: Spacing.md) {
            ...
            HStack(spacing: Spacing.xl) {
                QueueMetric(title: "Queued", value: stats.queuedRequests, color: .warning)
                QueueMetric(title: "Processing", value: stats.processingRequests, color: .info)
                QueueMetric(title: "Completed", value: stats.completedRequests, color: .success)
                QueueMetric(title: "Failed", value: stats.failedRequests, color: .error)
            }
        }
        .padding(Spacing.lg)
        .background(Color.surfaceSecondary.opacity(0.5))
    ...
    private var queueColor: Color {
        if requestQueue.queuedRequests == 0 { return .success }
    ...
    private var requestQueueMetrics: some View {
        VStack(alignment: .leading, spacing: Spacing.md) {
            ...
            VStack(spacing: Spacing.sm) {
                MetricDetailRow(title: "Success Rate", value: "\(stats.successRate * 100, specifier: "%.1f")%")
                MetricDetailRow(title: "Avg Processing Time", value: "\(stats.averageProcessingTime * 1000, specifier: "%.0f")ms")
                MetricDetailRow(title: "Queue Utilization", value: "\(stats.queueUtilization * 100, specifier: "%.1f")%")
                MetricDetailRow(title: "Processing Utilization", value: "\(stats.processingUtilization * 100, specifier: "%.1f")%")
            }
        }
        .padding(Spacing.lg)
        .background(Color.surfaceSecondary.opacity(0.5))
